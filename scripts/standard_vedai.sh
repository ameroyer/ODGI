...
#SBATCH -o ./log/output_logs/standard-vedai-$SIZE-$NUM_EPOCHS.%j.out     # logfile for stdout
#SBATCH -e ./log/error_logs/standard-vedai-$SIZE-$NUM_EPOCHS.%j.err      # logfile for stderr

module load tensorflow/python3/1.4.0      # enable tensorflow
cd ${HOME}/Jupyter/ODGI                   # working directory

python3 -u train_standard.py 'vedai'                        \
    --size=$SIZE                                            \
