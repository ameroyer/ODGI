...
python3 -u train_odgi.py $DATA --size $SIZE --num_epochs=$NUM_EPOCHS --num_gpus=$NUM_GPUS --display_loss_very_n_steps=$DISPLAY_LOSS_EVERY_N_STEPS --batch_size=$BATCH_SIZE --full_image_size=$FULL_IMAGE_SIZE