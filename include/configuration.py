...
_defaults_dict = {
    ...
    "grouping_method": "intersect",                        # Criterion to determine whether an object "belongs" to a cell
    ...
    "num_threads": 4,                                      # Number of parallel readers for the dataset map operationa
...
def start_from_pretrained(sess, verbose=False):
    """Use pretrained weights for training if available. For instance, to start
       the MobileNet backbone training from ImageNet pretrained model.
       Checkpoints from https://github.com/tensorflow/models/tree/master/research/slim/nets/mobilenet"""
    # Load saver object. See nets.mobilenet for an example
    saver_collections = [x for x in tf.get_default_graph().get_all_collection_keys() if x.endswith('_saver')]
    # Load the mobilenet checkpoint
...
def build_base_config_from_args(args, verbose=0):
    ...
    configuration['image_format'] = 'vedai' if args.data.startswith('vedai') else args.data

    ## Set default configuration for each dataset
    ...
    # Choose number of crops to maximize GPU memory usage
...
def finalize_grid_offsets(configuration, verbose=2):
    """Setthe current number of cells and grid offset, given the
    ...
    if network == 'tiny_yolo_v2':
        configuration['num_cells'] = _get_num_cells(image_size, 5)
...
def _get_num_cells(image_size, num_layers):
